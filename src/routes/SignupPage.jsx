...
function SignupPage() {
  ...
  async function handleSubmit(event) {
    ...
    // If Supabase is configured to require email confirmation, there will be
    // no active session yet — show a "check your inbox" state instead of
  ...
  if (confirmationNeeded) {
    return (
      <div className="mx-auto max-w-md px-4 py-16 text-center">
        <h1 className="font-serif text-3xl font-semibold text-ink">Check your email</h1>
        <p className="mt-4 text-slate-600">
          We sent a confirmation link to <span className="font-medium text-ink">{email}</span>.
          Confirm your address, then log in to your new MedPublish account.
        </p>
        <Link
          to="/login"
          className="mt-8 inline-flex items-center justify-center rounded-lg bg-ink px-5 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-ink-light"
        >
          Go to log in
        </Link>
      </div>
  ...
  return (
    <div className="mx-auto max-w-md px-4 py-16">
      <h1 className="font-serif text-3xl font-semibold text-ink">Sign Up</h1>
      <p className="mt-3 text-slate-600">
        Create an account to submit manuscripts and follow their editorial status.
      </p>
